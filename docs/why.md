...
`go-ruby-semantic-puppet/semantic-puppet` reimplements Ruby's `semantic-puppet` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-semantic-puppet/semantic-puppet` directly, with no Ruby runtime;
...
Because `semantic-puppet` is pure Go with cgo disabled, it also compiles to **WebAssembly**
